...
esvo2_core::core::EventBM::EventBM(esvo2_core::CameraSystem::Ptr camSysPtr,
                                   size_t numThread, bool bSmoothTS,
                                   size_t patch_size_X, size_t patch_size_Y,
                                   size_t min_disparity, size_t max_disparity,
                                   size_t step, double ZNCC_Threshold,
...
void esvo2_core::core::EventBM::resetParameters(
    size_t patch_size_X, size_t patch_size_Y, size_t min_disparity,
...
void esvo2_core::core::EventBM::createMatchProblem(
    constStampedTimeSurfaceObs *pStampedTsObs, StampTransformationMap *pSt_map,
  ...
  vEventsPtr_.insert(vEventsPtr_.end(), pvEventsPtr->begin(),
                     pvEventsPtr->end());
  ...
  for (size_t i = 0; i < vEventsPtr_.size(); i++)
    vpDisparitySearchBound_.push_back(
        std::make_pair(min_disparity_, max_disparity_));
...
void esvo2_core::core::EventBM::createMatchProblemTwoFrames(
    constStampedTimeSurfaceObs *pStampedTsObs, StampTransformationMap *pSt_map,
    std::vector<Event *> *pvEventsPtr, std::vector<EventMatchPair> *vEMP_lr) {
  pStampedTsObs_ = pStampedTsObs;
  pSt_map_ = pSt_map;
  ...
  for (size_t i = 0; i < vEventsPtr_.size(); i++)
    vpDisparitySearchBound_.push_back(
        std::make_pair(min_disparity_, max_disparity_));
  T_last_now_ =
      pStampedTsObs_->second.tr_last_.getTransformationMatrix().inverse() *
...
bool esvo2_core::core::EventBM::match_an_event2(
    const Event *pEvent, std::pair<size_t, size_t> &pDisparityBound,
  ...
  Eigen::Vector2d x_rect =
      camSysPtr_->cam_left_ptr_->getRectifiedUndistortedCoordinate(pEvent->x,
                                                                   pEvent->y);

  // check if the rectified and undistorted coordinates are outside the image
  // plane. (Added by Yi Zhou on 12 Jan 2021)
  ...
  if (camSysPtr_->cam_left_ptr_->UndistortRectify_mask_(
          (int)x_rect(1), (int)x_rect(0)) <= 125) {
    infoNoiseRatioLowNum_++;
    return false;
  }
  ...
  Eigen::MatrixXd patch_dst =
      Eigen::MatrixXd::Zero(patch_size_Y_, patch_size_X_);
  ...
  // for zncc speed up, from "Optimizing ZNCC calculation in binocular stereo
  // matching " mean_l is the mean sum of left patch, Tl_square is the sum of
  // square of the right patch.
  ...
  if (use_fast_zncc_) {
    ...
    patch_r = pStampedTsObs_->second.TS_right_.block(
        x1_left_top(1), x1_left_top(0) - (int)upDisparity, patch_size_Y_,
        patch_size_X_ + (int)upDisparity);

    colSum = patch_r.colwise().sum();

    colSquareSum = (patch_r.array().pow(2)).colwise().sum();

    for (int m = lowDisparity;
         m < lowDisparity + patch_src.cols() && (colSum.size() - 1 - m >= 0);
         m++) {
  ...
  // To improve search efficiency, static stereo block matching performs
  // searches at a certain step size and conducts additional searches near
  // points that preliminarily match.
  if (!epipolarSearchingCoarse(
          min_cost, bestMatch, bestDisp, patch_dst, lowDisparity, upDisparity,
          step_, x1, patch_src, bUpDownConfiguration_, searching_or_not, colSum,
          colSquareSum, mean_l, Tl_square, Tr, Tr_square)) {
  ...
  if (min_cost <= ZNCC_Threshold_ * 1.02) {
    ...
    emPair.x_right_ =
        Eigen::Vector2d((double)bestMatch(0), (double)bestMatch(1));
    emPair.t_ = pEvent->ts;
    double disparity;
    ...
    double depth =
        camSysPtr_->baseline_ * camSysPtr_->cam_left_ptr_->P_(0, 0) / disparity;
...
bool esvo2_core::core::EventBM::match_an_event(
    Event *pEvent, std::pair<size_t, size_t> &pDisparityBound,
    esvo2_core::core::EventMatchPair &emPair) {
  ...
  if (x_rect(0) < 0 || x_rect(0) > camSysPtr_->cam_left_ptr_->width_ - 1 ||
    ...
    return false;
  // This is to avoid depth estimation happening in the mask area.
  if (camSysPtr_->cam_left_ptr_->UndistortRectify_mask_((int)x_rect(1),
                                                        (int)x_rect(0)) <= 125)
    return false;
  Eigen::Vector2i x1(std::floor(x_rect(0)), std::floor(x_rect(1)));
  Eigen::Vector2i x1_left_top;
  ...
  Eigen::MatrixXd patch_dst =
      Eigen::MatrixXd::Zero(patch_size_Y_, patch_size_X_);
  // coarse searching
  if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst, lowDisparity,
                         upDisparity, step_, x1, patch_src,
                         bUpDownConfiguration_)) {
    coarseSearchingFailNum_++;
    return false;
  }
  // fine searching
  size_t fine_searching_start_pos =
      bestDisp - (step_ - 1) >= 0 ? bestDisp - (step_ - 1) : 0;
  if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst,
    ...
    // This indicates the local minima is not surrounded by two neighbors with
    // larger cost, This case happens when the best match locates over/outside
    // the boundary of the Time Surface.
    fineSearchingFailNum_++;
    return false;
  }
  ...
  if (min_cost <= ZNCC_Threshold_) {
    ...
    // auto st_map_iter = tools::StampTransformationMap_lower_bound(*pSt_map_,
    // emPair.t_); if(st_map_iter == pSt_map_->end())
    //   return false;
    ...
    // LOG(INFO) << "BM fails because: " << min_cost << " > " <<
    // ZNCC_Threshold_;
    return false;
  }
}
...
bool esvo2_core::core::EventBM::match_an_eventTwoFrames(
    EventMatchPair EMP_lr, std::pair<size_t, size_t> &pDisparityBound,
    esvo2_core::core::EventMatchPair &emPair) {
  int lowDisparity = -10;
  int upDisparity = 70;
  ...
  // Eigen::Vector2i x1(std::floor(EMP_lr.x_last_(0)),
  // std::floor(EMP_lr.x_last_(1)));
  Eigen::Vector2i x1(std::floor(x_rect(0)), std::floor(x_rect(1)));
  Eigen::Vector2i x1_left_top;
  if (!isValidPatch(x1, x1_left_top, patch_size_X_2_, patch_size_Y_2_)) {
    infoNoiseRatioLowNum_++;
  ...
  int bestDisp;
  Eigen::MatrixXd patch_dst =
      Eigen::MatrixXd::Zero(patch_size_X_2_, patch_size_Y_2_);
  ...
  if (!epipolarSearchingTwoFrames(min_cost, bestMatch, bestDisp, patch_dst,
                                  lowDisparity, upDisparity, 1, x1, patch_src,
                                  dx, dy, emPair.costs_, fine_search,
                                  bUpDownConfiguration_)) {
    coarseSearchingFailNum_++;
    return false;
  }
  ...
  if (min_cost <= ZNCC_Threshold_ * 3.00) {
    ...
    double disparity;
    if (bUpDownConfiguration_)
      disparity = sqrt(pow(emPair.x_left_(0) - bestMatch(0), 2) +
                       pow(emPair.x_left_(1) - bestMatch(1), 2));
    else
      disparity = sqrt(pow(emPair.x_left_(0) - bestMatch(0), 2) +
                       pow(emPair.x_left_(1) - bestMatch(1), 2));
    ...
    double depth =
        triangulatePoint(emPair.x_left_, emPair.x_last_, emPair.error_);
    if (depth < 0.8 || depth > 40)
      return false;

    emPair.x_right_ << EMP_lr.x_left_(0) -
                           camSysPtr_->baseline_ *
                               camSysPtr_->cam_left_ptr_->P_(0, 0) / depth,
        EMP_lr.x_left_(1);
    emPair.trans_ = emPair.trans_ = pStampedTsObs_->second.tr_;
    emPair.invDepth_ = 1.0 / depth;
    emPair.cost_ = min_cost;
...
bool esvo2_core::core::EventBM::epipolarSearching(
  ...
  for (size_t disp = searching_start_pos; disp <= searching_end_pos;
       disp += searching_step) {
    Eigen::Vector2i x2;
...
bool esvo2_core::core::EventBM::epipolarSearchingCoarse(
    ...
    size_t searching_end_pos, size_t searching_step, Eigen::Vector2i &x1,
    Eigen::MatrixXd &patch_src, bool bUpDownConfiguration,
    vector<bool> &searching_or_not, Eigen::VectorXd &colSum,
    Eigen::VectorXd &colSquareSum, double &mean_l, double &Tl_square,
  ...
  for (size_t disp = searching_start_pos; disp <= searching_end_pos;
    ...
    if (use_fast_zncc_) {
      ...
      cost =
          zncc_cost_fast(colSum, colSquareSum, patch_src, patch_dst, disp_to_rm,
                         step_to_rm, mean_l, Tl_square, Tr, Tr_square);
    } else {
...
bool esvo2_core::core::EventBM::epipolarSearchingFine(
    double &min_cost, Eigen::Vector2i &bestMatch, size_t &bestDisp,
  ...
  double var_l = (patch_src.array() * patch_src.array()).sum() -
                 patch_src.rows() * patch_src.cols() * mean_l * mean_l;
  for (int i = 0; i < searching_radius.size(); i++) {
    ...
    patch_dst = pStampedTsObs_->second.TS_right_.block(
        x2_left_top(1), x2_left_top(0), patch_size_Y_, patch_size_X_);
    double cost = ZNCC_MAX_;
    cost = zncc_cost2(patch_src, patch_dst, var_l,
                      mean_l); // zncc_cost(patch_src, patch_dst, true);
    if (cost <= min_cost) {
      min_cost = cost;
      bestMatch = x2;
      bestDisp = disp;
...
bool esvo2_core::core::EventBM::epipolarSearchingTwoFrames(
    double &min_cost, Eigen::Vector2i &bestMatch, int &bestDisp,
  ...
  if (!use_zncc) {
    ...
    rect1 = cv::Rect(x1_left_top(0), x1_left_top(1), patch_size_Y_2_,
                     patch_size_X_2_);
  ...
  for (int disp = searching_start_pos; disp <= searching_end_pos;
       disp += searching_step) {
    ...
    if (use_zncc) {
      ...
      // ssim
      rect2 = cv::Rect(x2_left_top(0), x2_left_top(1), patch_size_Y_2_,
                       patch_size_X_2_);
...
void esvo2_core::core::EventBM::match_all_SingleThreadTwoFrames(
    std::vector<EventMatchPair> &vEMP, std::vector<EventMatchPair> &vEMP_fail) {
  TicToc t;
  t.tic();
  coarseSearchingFailNum_ = 0;
  fineSearchingFailNum_ = 0;
...
bool esvo2_core::core::EventBM::isValidPatch(Eigen::Vector2i &x,
...
void esvo2_core::core::EventBM::match2(EventBM::Job &job) {
  ...
  for (size_t i = i_thread; i < totalNumEvents;
    ...
    std::pair<size_t, size_t> pDisparityBound =
        (*job.pvpDisparitySearchBound_)[i];
...
double esvo2_core::core::EventBM::zncc_cost(Eigen::MatrixXd &patch_left,
  ...
  if (!normalized) {
    ...
    cost = 0.5 *
           (1 - (patch_left_normalized.array() * patch_right_normalized.array())
                        .sum() /
...
double esvo2_core::core::EventBM::zncc_cost_fast(
    Eigen::VectorXd &colSum, Eigen::VectorXd &colSquareSum,
    Eigen::MatrixXd &patch_left, Eigen::MatrixXd &patch_right, int &disp_to_rm,
    int &step_to_rm, double &mean_l, double &Tl_square, double &Tr,
    double &Tr_square) {
  ...
  if (abs(mean_l - mean_r) / mean_l > 5 || abs(mean_l - mean_r) / mean_r > 5) {
    ...
    var_l =
        Tl_square - patch_right.rows() * patch_right.cols() * mean_l * mean_l;
  ...
  for (int i = 0; i < step_to_rm; i++) {
    if ((int)colSum.size() - 1 - disp_to_rm - patch_right.cols() > 0) {
      ...
      Tr_square =
          Tr_square - colSquareSum[colSum.size() - 1 - disp_to_rm] +
...
double esvo2_core::core::EventBM::zncc_cost2(Eigen::MatrixXd &patch_left,
                                             Eigen::MatrixXd &patch_right,
                                             double &var_l, double &mean_l) {
  double cost, cov, var_r, var_l_ori;
  double mean_r = patch_right.array().mean();
  cov = (patch_left.array() * patch_right.array()).sum() -
        patch_right.rows() * patch_right.cols() * mean_l * mean_r;
  var_r = (patch_right.array() * patch_right.array()).sum() -
          patch_right.rows() * patch_right.cols() * mean_r * mean_r;
  if (var_l * var_r == 0)
    cost = 0;
  else
    cost = cov / sqrt(var_l * var_r);
...
float esvo2_core::core::EventBM::getMSSIM(const cv::Mat &i1,
                                          const cv::Mat &i2) {
...
double esvo2_core::core::EventBM::triangulatePoint(Eigen::Vector2d &point0,
                                                   Eigen::Vector2d &point1,
                                                   double depth) {
  Eigen::Matrix<double, 3, 4> Pose0 =
      Eigen::Matrix4d::Identity().block(0, 0, 3, 4);
