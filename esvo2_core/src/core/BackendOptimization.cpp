...
void BackendOptimization::setProblem(
    std::deque<DepthPointFrame> *pDepthPoints, TimeSurfaceHistory *pTS_history,
    rclcpp::Publisher<RepackagingMsg> *pV_ba_bg_pub, bool bUSE_IMU) {
...
void BackendOptimization::sloveProblem() {
  ...
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    const auto tt =
        (*pDepthPoints_)[dqvDepthPoints_size - WINDOW_SIZE - 1 + i].timestamp_;
  ...
  // if the solver not converged, update the Vs, Bas, Bgs from the last
  // iteration
  ...
  if (initVsFlag && summary.termination_type == ceres::CONVERGENCE)
    publishVBaBg((*pDepthPoints_)[(*pDepthPoints_).size() - 1].timestamp_);
...
void BackendOptimization::vector2Double(
    esvo2_core::container::TimeSurfaceHistory::iterator ts_obs,
    esvo2_core::container::TimeSurfaceHistory::iterator last_obs,
...
void BackendOptimization::Tcam2Timu(
    esvo2_core::container::TimeSurfaceHistory::iterator ts_obs,
    esvo2_core::container::TimeSurfaceHistory::iterator last_obs,
    double para_Pose[][7], int i) {
  if (i == 0) {
    ...
    Eigen::Matrix4d T_imu =
        T_i_c_ * T_wopt_window0_.inverse() * T_wopt_cam * T_i_c_.inverse();
    ...
    T_window0_i = T_i_c_ * T_wopt_window0_.inverse() * T_wopt_last *
                  T_wori_last.inverse() * T_wori_c * T_i_c_.inverse();
...
void BackendOptimization::double2Vector(double para_Pose[][7],
  ...
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    Rs[i] = R_wopt_window0 * RIC_.inverse() *
            Eigen::Quaterniond(para_Pose[i][6], para_Pose[i][3],
                               para_Pose[i][4], para_Pose[i][5])
                .toRotationMatrix() *
            RIC_;
    Ps[i] = R_wopt_window0 * RIC_.inverse() *
                Eigen::Vector3d(para_Pose[i][0] - para_Pose[0][0],
                                para_Pose[i][1] - para_Pose[0][1],
                                para_Pose[i][2] - para_Pose[0][2]) +
            origin_P0;
    // if initialized Vs, Bas, Bgs, update them
    if (initVsFlag) {
      Vs[i] = Eigen::Vector3d(para_SpeedBias[i][0], para_SpeedBias[i][1],
                              para_SpeedBias[i][2]);
    ...
    Transformation Tr(T);
    const auto tt =
        (*pDepthPoints_)[(*pDepthPoints_).size() - WINDOW_SIZE - 1 + i]
            .timestamp_;
    auto ts_obs = (*pTS_history_).find(tt);
...
void BackendOptimization::publishVBaBg(double time_v) {
  events_repackaging_msgs::msg::Vbabg msg;
...
bool BackendOptimization::CalibrationExRotation(
  ...
  for (int i = 1; i <= WINDOW_SIZE; i++) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("backend"), "%d %f", i, angular_distance);
...
void BackendOptimization::solveGyroscopeBias(Eigen::Vector3d &Bgs) {
  ...
  Bgs = A.ldlt().solve(b);
  RCLCPP_WARN_STREAM(rclcpp::get_logger("backend"),
                     "gyroscope bias initial calibration " << Bgs.transpose());
}

bool BackendOptimization::LinearAlignment(Eigen::Vector3d &g,
                                          Eigen::VectorXd &x) {
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    tmp_A.block<3, 3>(0, 6) =
        Rs[i].transpose() * dt * dt / 2 * Eigen::Matrix3d::Identity();
    tmp_A.block<3, 1>(0, 9) = Rs[i].transpose() * (Ps[j] - Ps[i]) / 100.0;
    tmp_b.block<3, 1>(0, 0) =
        pre_integrations[j]->delta_p + Rs[i].transpose() * Rs[j] * TIC_ - TIC_;
    tmp_A.block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();
    tmp_A.block<3, 3>(3, 3) = Rs[i].transpose() * Rs[j];
    tmp_A.block<3, 3>(3, 6) =
        Rs[i].transpose() * dt * Eigen::Matrix3d::Identity();
...
void BackendOptimization::RefineGravity(Eigen::Vector3d &g,
                                        Eigen::VectorXd &x) {
  ...
  for (int k = 0; k < 4; k++) {
    ...
    for (; i < WINDOW_SIZE; i++) {
      ...
      tmp_A.block<3, 3>(0, 0) = -dt * Eigen::Matrix3d::Identity();
      tmp_A.block<3, 2>(0, 6) =
          Rs[i].transpose() * dt * dt / 2 * Eigen::Matrix3d::Identity() * lxly;
      tmp_A.block<3, 1>(0, 8) = Rs[i].transpose() * (Ps[i + 1] - Ps[i]) / 100.0;
      tmp_b.block<3, 1>(0, 0) = pre_integrations[i]->delta_p +
                                Rs[i].transpose() * Rs[i + 1] * TIC_ - TIC_ -
                                Rs[i].transpose() * dt * dt / 2 * g0;

      tmp_A.block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();
      tmp_A.block<3, 3>(3, 3) = Rs[i].transpose() * Rs[i + 1];
      tmp_A.block<3, 2>(3, 6) =
          Rs[i].transpose() * dt * Eigen::Matrix3d::Identity() * lxly;
      tmp_b.block<3, 1>(3, 0) =
          pre_integrations[i + 1]->delta_v -
          Rs[i].transpose() * dt * Eigen::Matrix3d::Identity() * g0;
  ...
  if (frame_count >= WINDOW_SIZE) {
    ...
    if (bUSE_IMU_) {
      ...
      pre_integrations[WINDOW_SIZE] = new IntegrationBase{
          acc_0, gyr_0, Bas[WINDOW_SIZE], Bgs[WINDOW_SIZE], g_optimal};
    }
  }
  if (frame_count < WINDOW_SIZE)
    frame_count++;
...
Eigen::Matrix3d
BackendOptimization::fixRotationMatrix(const Eigen::Matrix3d &R) {
