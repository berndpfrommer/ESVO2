...
void Visualization::plot_map(DepthMap::Ptr &depthMapPtr, VisMapType vmType,
                             cv::Mat &img, double max_range, double min_range,
  ...
  case InvDepthMap_rel: {
    // cv::Mat img_old = cv::imread("/home/bell/data/rel.tiff",
    // cv::IMREAD_UNCHANGED);
...
void Visualization::DrawPoint(double val, double max_range, double min_range,
  ...
  cv::Scalar color =
      CV_RGB(255.0f * r[index], 255.0f * g[index], 255.0f * b[index]);
  // draw the point
  cv::Point point;
  point.x = location[0];
  point.y = location[1];
...
void Visualization::DrawPoint2(double val, double max_range, double min_range,
                               const Eigen::Vector2d &location, cv::Mat &img) {
  // define the color based on the inverse depth
  //  int index = floor((val - min_range) / (max_range - min_range) * 255.0f);
...
void Visualization::plot_eventMap(std::vector<Event *> &vEventPtr,
                                  cv::Mat &eventMap, size_t row, size_t col) {
  eventMap = cv::Mat(cv::Size(col, row), CV_8UC1, cv::Scalar(0));
  for (int i = 0; i < vEventPtr.size(); i++) {
    if (vEventPtr[i]->x >= 0 && vEventPtr[i]->x < col && vEventPtr[i]->y >= 0 &&
        vEventPtr[i]->y < row)
