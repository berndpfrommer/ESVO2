...
void PerspectiveCamera::setIntrinsicParameters(
    size_t width, size_t height, std::string &cameraName,
    std::string &distortion_model, std::vector<double> &vD,
    std::vector<double> &vK, std::vector<double> &vRectMat,
    std::vector<double> &vP, std::vector<double> &vT_b_c) {
  width_ = width;
  height_ = height;
...
void PerspectiveCamera::preComputeRectifiedCoordinate() {
  ...
  if (distortion_model_ == "plumb_bob") {
    cv::undistortPoints(RawCoordinates, RectCoordinates, cvKmat, cvDistCoeff,
                        cvRectMat, cvPmat);
    ...
    cv::remap(cvSrcMask, cvDstMask, undistort_map1_, undistort_map2_,
              cv::INTER_LINEAR);
...
#else
    ROS_ERROR_ONCE(
        "You need OpenCV >= 3.0 to use the equidistant camera model.");
    ROS_ERROR_ONCE("Will not publish rectified images.");
#endif
  } else if (distortion_model_ == "equidistant") {
    cv::fisheye::undistortPoints(RawCoordinates, RectCoordinates, cvKmat,
                                 cvDistCoeff, cvRectMat, cvPmat);
#if CV_MAJOR_VERSION >= 3
    cv::Size sensor_size(width_, height_);
...
void PerspectiveCamera::cam2World(const Eigen::Vector2d &x, double invDepth,
                                  Eigen::Vector3d &p) {
  ...
  p = p_s.block<3, 1>(0, 0) /
      p_s(3); // This normalization by the last element is not necessary.
}

void PerspectiveCamera::world2Cam(const Eigen::Vector3d &p,
                                  Eigen::Vector2d &x) {
...
/************************************************************/
/************************************************************/
CameraSystem::CameraSystem(const std::string &calibInfoDir,
                           bool bPrintCalibInfo) {
...
void CameraSystem::loadCalibInfo(const std::string &cameraSystemDir,
                                 bool bPrintCalibInfo) {
  ...
  std::string cameraNameLeft =
      leftCamCalibInfo["camera_name"].as<std::string>();
  std::string cameraNameRight =
      rightCamCalibInfo["camera_name"].as<std::string>();
  std::string distortion_model =
      leftCamCalibInfo["distortion_model"].as<std::string>();
  ...
  vD_left = leftCamCalibInfo["distortion_coefficients"]["data"]
                .as<std::vector<double>>();
  vK_left = leftCamCalibInfo["camera_matrix"]["data"].as<std::vector<double>>();
  vRectMat_left = leftCamCalibInfo["rectification_matrix"]["data"]
                      .as<std::vector<double>>();
  vP_left =
      leftCamCalibInfo["projection_matrix"]["data"].as<std::vector<double>>();

  vD_right = rightCamCalibInfo["distortion_coefficients"]["data"]
                 .as<std::vector<double>>();
  vK_right =
      rightCamCalibInfo["camera_matrix"]["data"].as<std::vector<double>>();
  vRectMat_right = rightCamCalibInfo["rectification_matrix"]["data"]
                       .as<std::vector<double>>();
  vP_right =
      rightCamCalibInfo["projection_matrix"]["data"].as<std::vector<double>>();

  vT_right_left =
      leftCamCalibInfo["T_right_left"]["data"].as<std::vector<double>>();
  vT_b_c = leftCamCalibInfo["T_b_c"]["data"].as<std::vector<double>>();

  cam_left_ptr_->setIntrinsicParameters(width, height, cameraNameLeft,
                                        distortion_model, vD_left, vK_left,
                                        vRectMat_left, vP_left, vT_b_c);
  cam_right_ptr_->setIntrinsicParameters(width, height, cameraNameRight,
                                         distortion_model, vD_right, vK_right,
                                         vRectMat_right, vP_right, vT_b_c);

  T_right_left_ =
      Eigen::Matrix<double, 3, 4, Eigen::RowMajor>(vT_right_left.data());
