...
poseToTransformStamped(const geometry_msgs::PoseStamped &pose_msg,
                       const std::string &child_frame_id) {
  geometry_msgs::msg::TransformStamped transform_msg;
  transform_msg.header.frame_id = pose_msg.header.frame_id;
  transform_msg.header.stamp.sec = pose_msg.header.stamp.sec;
  transform_msg.header.stamp.nanosec = pose_msg.header.stamp.nsec;
...
tf2::TimePoint toTf2TimePoint(const ros::Time &stamp) {
  return tf2::timeFromSec(stamp.toSec());
}

Transformation transformStampedToKindr(
...
esvo2_Tracking::esvo2_Tracking(const ros::NodeHandle &nh,
                               const ros::NodeHandle &nh_private)
    : nh_(nh), pnh_(nh_private), it_(nh),
      ...
      TS_negaTS_sync_(ApproximateSyncPolicy_negaTS(10), TS_left_sub_,
      ...
      rpType_(
          (RegProblemType)((size_t)tools::param(pnh_, "RegProblemType", 0))),
      rpSolver_(camSysPtr_, rpConfigPtr_, rpType_, NUM_THREAD_TRACKING),
      ESVO2_System_Status_("INITIALIZATION"),
      imu_data_(Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), g_optimal),
      ets_(IDLE) {
  // offline data
  ...
  TS_negaTS_sync_.registerCallback(boost::bind(
      &esvo2_Tracking::timeSurface_NegaTS_Callback, this, _1, _2, _3, _4));

  tf_ = std::make_shared<tf2::BufferCore>(tf2::durationFromSec(100.0));
  pose_pub_ =
      nh_.advertise<geometry_msgs::PoseStamped>("/esvo2_tracking/pose_pub", 1);
  path_pub_ = nh_.advertise<nav_msgs::Path>("/esvo2_tracking/trajectory", 1);
  map_sub_ = nh_.subscribe("pointcloud", 0, &esvo2_Tracking::refMapCallback,
                           this); // local map in the ref view.
  stampedPose_sub_ =
      nh_.subscribe("stamped_pose", 0, &esvo2_Tracking::stampedPoseCallback,
                    this); // for accessing the pose of the ref view.
  imu_sub_ = nh_.subscribe("/imu/data", 0, &esvo2_Tracking::refImuCallback,
                           this); // local map in the ref view.
  V_ba_bg_sub_ = nh_.subscribe("/esvo2_mapping/V_ba_bg", 0,
                               &esvo2_Tracking::VBaBgCallback, this);
...
void esvo2_Tracking::TrackingLoop() {
  ...
  while (ros::ok()) {
    ...
    if (ESVO2_System_Status_ == "INITIALIZATION" &&
        ets_ == WORKING) // This is true when the system is reset from dynamic
                         // reconfigure
    {
      reset();
      r.sleep();
    ...
    // Data Transfer (If mapping node had published refPC.)
    {
      ...
      if (ref_.t_.toSec() <
          refPCMap_.rbegin()->first.toSec()) // new reference map arrived
        refDataTransferring();
      if (cur_.t_.toSec() <
          TS_history_.rbegin()->first.toSec()) // new observation arrived
      {
        if (ref_.t_.toSec() >= TS_history_.rbegin()->first.toSec()) {
          LOG(INFO) << "The time_surface observation should be obtained after "
                       "the reference frame";
          exit(-1);
        }
    ...
    LOG(INFO) << "Registration: " << t_solve << " ms, ("
...
bool esvo2_Tracking::refDataTransferring() {
  ...
  if (ESVO2_System_Status_ == "WORKING" ||
      (ESVO2_System_Status_ == "INITIALIZATION" && ets_ == WORKING)) {
    if (!getPoseAt(t, ref_.tr_, dvs_frame_id_)) {
      LOG(INFO) << "ESVO2_System_Status_: " << ESVO2_System_Status_
                << ", ref_.t_: " << ref_.t_.toNSec();
      LOG(INFO) << "Logic error ! There must be a pose for the given "
                   "timestamp, because mapping has been finished.";
      // exit(-1);
      return false;
    }
  }
  ...
  while (PointXYZ_begin_it != PointXYZ_end_it) {
    ref_.vPointXYZPtr_.push_back(
        PointXYZ_begin_it.base()); // Copy the pointer of the pointXYZ
    PointXYZ_begin_it++;
  }
  return true;
}
...
bool esvo2_Tracking::curDataTransferring() {
  ...
  // TS_history may not be updated before the tracking loop excutes the data
  // transfering
  ...
  if (ESVO2_System_Status_ == "WORKING" ||
      (ESVO2_System_Status_ == "INITIALIZATION" && ets_ == WORKING)) {
    curImuTransferring();
    // if use imu, the pose of the current frame is updated in the imu
    // preintegration.
    if (bUseImu_) {
      ...
      if (t_world_cur_ != Eigen::Vector3d::Zero() &&
          last_t_world_cur_ != Eigen::Vector3d::Zero()) {
        ...
        if (qprevious_ts_.size() > average_window_size) {
          qprevious_ts_.erase(qprevious_ts_.begin(), qprevious_ts_.begin() +
                                                         qprevious_ts_.size() -
                                                         average_window_size);
        }
        ...
        // If the predicted position change is significantly different from the
        // previous displacement due to potentially unstable velocity estimates,
        // use the previous displacement as the initial value for the next
        // optimization.
        if (initVsFlag &&
            (imu_data_.t_v_last_mapping.second * imu_data_.sum_dt - last_t_)
                        .norm() /
                    last_t_.norm() <
                0.1)
          T_world_cur_.block(0, 3, 3, 1) +=
              R_b_c_.transpose() * Imu_t +
              (imu_data_.t_v_last_mapping.second * imu_data_.sum_dt);
        else
          T_world_cur_.block(0, 3, 3, 1) +=
              R_b_c_.transpose() * Imu_t + last_t_;
      } else {
        last_t_world_cur_ = T_world_cur_.block(0, 3, 3, 1);
      }

      T_world_cur_.block(0, 0, 3, 3) =
          R_w_c * R_b_c_ * q.toRotationMatrix() * R_b_c_.inverse();
    }
    Eigen::Matrix3d R_w_c = T_world_cur_.block(0, 0, 3, 3);
...
bool esvo2_Tracking::curImuTransferring() {
  ...
  if (bUseImu_) {
    imu_mutex_.lock();
    if (initVsFlag &&
        (refPCMap_.rbegin()->first.toSec() - imu_data_.t_v_last_mapping.first >
         0.001))
    ...
    if (t_world_cur_ != Eigen::Vector3d::Zero() &&
        last_t_world_cur_ != Eigen::Vector3d::Zero()) {
      Eigen::Matrix3d R_w_c = T_world_cur_.block(0, 0, 3, 3);
      imu_data_.t_v_last_mapping.second +=
          R_b_c_ * R_w_c.transpose() * imu_data_.delta_v;
    }
    imu_mutex_.unlock();
  }
...
void esvo2_Tracking::VBaBgCallback(
    const events_repackaging_msgs::msg::Vbabg &msg) {
...
void esvo2_Tracking::refMapCallback(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
  pcl::PCLPointCloud2 pcl_pc;
  pcl_conversions::toPCL(*msg, pcl_pc);
  pcl::PointCloud<pcl::PointXYZRGBL>::Ptr PC_ptr(
...
void esvo2_Tracking::eventsCallback(const dvs_msgs::EventArray::ConstPtr &msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  for (const dvs_msgs::Event &e : msg->events) {
    ...
    while (i >= 0 &&
           events_left_[i].ts >
               e.ts) // we may have to sort the queue, just in case the raw
...
void esvo2_Tracking::clearEventQueue() {
  ...
  if (events_left_.size() > MAX_EVENT_QUEUE_LENGTH) {
    ...
    events_left_.erase(events_left_.begin(),
                       events_left_.begin() + remove_events);
  }
}

void esvo2_Tracking::timeSurface_NegaTS_Callback(
  ...
  try {
    ...
    cv_ptr_negative = cv_bridge::toCvCopy(time_surface_negative,
                                          sensor_msgs::image_encodings::MONO8);
    cv_ptr_dx = cv_bridge::toCvCopy(time_surface_dx,
                                    sensor_msgs::image_encodings::TYPE_16SC1);
    cv_ptr_dy = cv_bridge::toCvCopy(time_surface_dy,
                                    sensor_msgs::image_encodings::TYPE_16SC1);
  ...
  TS_history_.emplace(
      t_new_ts, TimeSurfaceObservation(cv_ptr_left, cv_ptr_negative, cv_ptr_dx,
                                       cv_ptr_dy, TS_id_, false));
  TS_id_++;
...
void esvo2_Tracking::stampedPoseCallback(
    const geometry_msgs::PoseStampedConstPtr &msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
  const auto transform_msg = poseToTransformStamped(*msg, dvs_frame_id_);
  tf_->setTransform(transform_msg, "esvo2_tracking");
}

bool esvo2_Tracking::getPoseAt(const ros::Time &t,
                               esvo2_core::Transformation &Tr,
...
void esvo2_Tracking::publishPose(const ros::Time &t, Transformation &tr) {
  ...
  if (!resultPath_.empty()) {
    ...
    f << t << " ";
    f.precision(5);
    f << ps_ptr->pose.position.x << " " << ps_ptr->pose.position.y << " "
      << ps_ptr->pose.position.z << " " << ps_ptr->pose.orientation.x << " "
      << ps_ptr->pose.orientation.y << " " << ps_ptr->pose.orientation.z << " "
...
void esvo2_Tracking::saveTrajectory(std::string &resultDir) {
  LOG(INFO) << "Saving trajectory to "
            << resultPath_ + "stamped_traj_estimate.txt" << " ......";
  ...
  std::list<Eigen::Matrix<double, 4, 4>,
            Eigen::aligned_allocator<Eigen::Matrix<double, 4, 4>>>::iterator
  ...
  for (; result_it_begin != result_it_end; result_it_begin++, ts_it_begin++) {
    ...
    f << *ts_it_begin << " ";
    f.precision(5);
    f << twc_result.transpose().x() << " " << twc_result.transpose().y() << " "
      << twc_result.transpose().z() << " " << q.x() << " " << q.y() << " "
      << q.z() << " " << q.w() << endl;
  }
  f.close();
  LOG(INFO) << "Saving trajectory to "
            << resultPath_ + "stamped_traj_estimate.txt" << ". Done !!!!!!.";
}
...
void esvo2_Tracking::groundTruthCallback(
    const geometry_msgs::PoseStampedConstPtr &msg) {
  std::ofstream f;
...
Eigen::Matrix3d esvo2_Tracking::fixRotationMatrix(const Eigen::Matrix3d &R) {
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(R, Eigen::ComputeFullU |
                                               Eigen::ComputeFullV);
