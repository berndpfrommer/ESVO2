...
struct DepthPointFrame {
  ...
  DepthPointFrame(timestamp_t t, std::vector<DepthPoint> DepthPoints) {
    timestamp_ = t;
    DepthPoints_ = DepthPoints;
  }

  int size() { return DepthPoints_.size(); }
};

class BackendOptimization {
public:
  using RepackagingMsg = events_repackaging_msgs::msg::Vbabg;
  ...
  void setProblem(std::deque<DepthPointFrame> *dqvDepthPoints,
                  ...
                  rclcpp::Publisher<RepackagingMsg> *pV_ba_bg_pub,
                  bool bUSE_IMU);
  void sloveProblem();
  ...
  void
  vector2Double(esvo2_core::container::TimeSurfaceHistory::iterator ts_obs,
                esvo2_core::container::TimeSurfaceHistory::iterator last_obs,
  ...
  // bool getPoseAt(const timestamp_t t, esvo2_core::Transformation &Tr, const
...
private:
  ...
  rclcpp::Publisher<RepackagingMsg> *pV_ba_bg_pub_;
