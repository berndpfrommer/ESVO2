#ifndef ESVO2_CORE_CONTAINER_CAMERASYSTEM_H
#define ESVO2_CORE_CONTAINER_CAMERASYSTEM_H

#include <Eigen/Dense>
#include <boost/shared_ptr.hpp>
#include <opencv2/core/core.hpp>
#include <string>
#include <yaml-cpp/yaml.h>

namespace esvo2_core {
namespace container {
class PerspectiveCamera {
public:
  PerspectiveCamera();
  virtual ~PerspectiveCamera();
  using Ptr = std::shared_ptr<PerspectiveCamera>;

  void
  setIntrinsicParameters(size_t width, size_t height, std::string &cameraName,
                         std::string &distortion_model, std::vector<double> &vD,
                         std::vector<double> &vK, std::vector<double> &vRectMat,
                         std::vector<double> &vP, std::vector<double> &vT_b_c);

  void preComputeRectifiedCoordinate();

  Eigen::Matrix<double, 2, 1> getRectifiedUndistortedCoordinate(int xcoor,
                                                                int ycoor);

  void cam2World(const Eigen::Vector2d &x, double invDepth, Eigen::Vector3d &p);

  void world2Cam(const Eigen::Vector3d &p, Eigen::Vector2d &x);

public:
  size_t width_, height_;
  std::string cameraName_;
  std::string distortion_model_;
  Eigen::Matrix<double, 4, 1> D_;
  Eigen::Matrix3d K_, K_inv_;
  Eigen::Matrix3d RectMat_;
  Eigen::Matrix<double, 3, 4> P_;
  Eigen::Matrix<double, 3, 4> T_b_c_;

  Eigen::Matrix2Xd precomputed_rectified_points_;
  cv::Mat undistort_map1_, undistort_map2_;
  Eigen::MatrixXi UndistortRectify_mask_;
};

class CameraSystem {
public:
  CameraSystem(const std::string &calibInfoDir, bool bPrintCalibInfo = false);
  virtual ~CameraSystem();
  using Ptr = std::shared_ptr<CameraSystem>;

  void computeBaseline();
  void loadCalibInfo(const std::string &cameraSystemDir,
                     bool bPrintCalibInfo = false);
  void printCalibInfo();

  PerspectiveCamera::Ptr cam_left_ptr_, cam_right_ptr_; // intrinsics
  Eigen::Matrix<double, 3, 4> T_right_left_;            // extrinsics
  double baseline_;
};
} // namespace container
} // namespace esvo2_core

#endif // ESVO2_CORE_CONTAINER_CAMERASYSTEM_H