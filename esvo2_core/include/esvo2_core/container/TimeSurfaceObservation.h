...
struct TimeSurfaceObservation {
  TimeSurfaceObservation(cv_bridge::CvImagePtr &left,
                         cv_bridge::CvImagePtr &right, Transformation &tr,
                         size_t id, bool bCalcTsGradient = false)
      : tr_(tr), id_(id) {
...
#ifdef TIME_SURFACE_OBSERVATION_LOG
      LOG(INFO) << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@Sobel computation (" << id_
                << ") takes " << tt.toc() << " ms.";
#endif
    }
    bSubNegaTS_ = false;
  ...
  TimeSurfaceObservation(cv_bridge::CvImagePtr &left,
                         ...
                         cv_bridge::CvImagePtr &negative_dy, size_t id,
  ...
  // override version without initializing the transformation in the
  // constructor.
  TimeSurfaceObservation(cv_bridge::CvImagePtr &left,
                         cv_bridge::CvImagePtr &right, size_t id,
                         bool bCalcTsGradient = false)
      : id_(id) {
    cvImagePtr_left_ = left;
  ...
  inline void setOriTransformation(Transformation &tr) { tr_ori_ = tr; }
  ...
  inline void GaussianBlurTS(size_t kernelSize) {
    ...
    if (TS_last_.rows() != 0 && TS_last_.cols() != 0 && TS_last_.rows() != 0 &&
        TS_last_.cols() != 0) {
  ...
  Eigen::MatrixXd TS_left_, TS_right_, TS_last_, AA_map_, TS_last_du,
      TS_last_dv;
  Eigen::MatrixXd TS_blurred_left_;
  Eigen::MatrixXd TS_negative_left_;
  cv_bridge::CvImagePtr cvImagePtr_left_, cvImagePtr_right_, cvImagePtr_last_,
      cvImagePtr_AA_map_;
  ...
  cv::Mat cv_TS_negative_left_, cv_dTS_negative_du_left_,
      cv_dTS_negative_dv_left_;
  size_t id_;
...
struct ROSTimeCmp {
  bool operator()(timestamp_t a, timestamp_t b) const { return a < b; }
};

using TimeSurfaceHistory =
    std::map<timestamp_t, TimeSurfaceObservation, ROSTimeCmp>;
using StampedTimeSurfaceObs = std::pair<timestamp_t, TimeSurfaceObservation>;
using constStampedTimeSurfaceObs =
    std::pair<const timestamp_t, TimeSurfaceObservation>;

inline static TimeSurfaceHistory::iterator
TSHistory_lower_bound(TimeSurfaceHistory &ts_history, timestamp_t t) {
  return std::lower_bound(
      ts_history.begin(), ts_history.end(), t,
      [](const std::pair<timestamp_t, TimeSurfaceObservation> &tso,
         timestamp_t t) { return tso.first < t; });
}

inline static TimeSurfaceHistory::iterator
TSHistory_upper_bound(TimeSurfaceHistory &ts_history, timestamp_t t) {
