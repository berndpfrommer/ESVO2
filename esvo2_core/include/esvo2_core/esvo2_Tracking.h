...
public:
  ...
  bool curDataTransferring(); // These two data transferring functions are
                              // decoupled because the data are not updated at
                              // the same frequency.
  ...
  void VBaBgCallback(const events_repackaging_msgs::msg::Vbabg &msg);
  ...
  // results
  void publishPose(const timestamp_t t, Transformation &tr);
  void publishPath(const timestamp_t t, Transformation &tr);
  ...
  bool getPoseAt(const timestamp_t t,
...
private:
  ...
  message_filters::Subscriber<sensor_msgs::Image> TS_negative_sub_, TS_dx_sub_,
      TS_dy_sub_;
  ...
  std::map<timestamp_t, pcl::PointCloud<pcl::PointXYZRGBL>::Ptr> refPCMap_;
  RefFrame ref_;
  CurFrame cur_;
